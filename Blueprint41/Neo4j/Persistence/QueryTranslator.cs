...
public abstract partial class QueryTranslator(Neo4jPersistenceProvider persistenceProvider)
{
    ...
    internal virtual void Compile(FieldResult field, CompileState state)
    {
        ...
        {
            var compiledArgs = field.FunctionArgs?.Select(arg => state.Preview(GetCompile(arg), state)).ToArray();
            string compiledText = string.Format(functionText.Replace("{base}", "{{base}}"), compiledArgs ?? []);

            if ((object?)field.Field is not null)
            {
                string[] split = compiledText.Split(separator, StringSplitOptions.None);
                if (split.Length == 0)
    ...
    internal virtual void Compile(AliasResult alias, CompileState state)
    {
        ...
        {
            var compiledArgs = alias.FunctionArgs?.Select(arg => state.Preview(GetCompile(arg), state)).ToArray();
            string compiledText = string.Format(functionText.Replace("{base}", "{{base}}"), compiledArgs ?? []);
    ...
    internal virtual void Compile(PathResult path, CompileState state) => Compile(path.Alias, state);

    internal virtual void Compile(Literal litheral, CompileState state) => state.Text.Append(litheral.Text);
    ...
    internal virtual void Compile(q.Query query, CompileState state)
    {
        switch (query.Type)
        {
            ...
            case PartType.OptionalMatch:
                ...
                q.Query.ForEach(query.Patterns, state.Text, ", ", item => item?.Compile(state));
                break;

            case PartType.UsingScan:
                state.Text.Append("USING SCAN ");
                q.Query.ForEach(query.Aliases, state.Text, "\r\nUSING SCAN ", item =>
                {
            ...
            case PartType.UsingIndex:
                ...
                q.Query.ForEach(query.Fields, state.Text, "\r\nUSING INDEX ", item =>
                {
            ...
            case PartType.OrderBy:
                ...
                q.Query.ForEach(query.Fields, state.Text, ", ", delegate (FieldResult item)
                {
                    item?.Compile(state);
                    if (!query.Ascending)
            ...
            case PartType.Return:
                ...
                q.Query.ForEach(query.AsResults, state.Text, ", ", item => item?.Compile(state));
                break;
            ...
            case PartType.OnCreate:
                ...
                q.Query.ForEach(query.Assignments, state.Text, ", ", item => item?.Compile(state, query.SetAdd));
                break;
            ...
            case PartType.Delete:
                ...
                q.Query.ForEach(query.Results, state.Text, ", ", item => item?.Compile(state));
                break;

            case PartType.Where:
                state.Text.Append("WHERE ");
                q.Query.ForEach(query.Conditions, state.Text, " AND ", item => item?.Compile(state));
                break;
    ...
    protected internal virtual void SearchTranslation(q.Query query, CompileState state)
    {
    ...
    public virtual string FnApocUtilMD5(int count) => $"apoc.util.md5([{string.Join(", ", Enumerable.Range(0, count).Select(item => string.Concat("{", item, "}")))}])";
    ...
    #endregion Compile Operators

    #region PersistenceProvider

    internal Neo4jPersistenceProvider PersistenceProvider { get; private set; } = persistenceProvider;
    ...
    #endregion Upgrade Script Parser

    #region Helper Methods

    protected static object? Substitute(CompileState state, object? operand)
    {
        if (operand is null)
            return null;
        ...
        switch (operand)
        {
            ...
            default:
                ...
                state.TypeMappings.TryGetValue(type, out var mapping);
                if (mapping is null)
                    return operand;
    ...
    protected static Type? GetOperandType(object? operand)
    {
        if (operand is null)
            return null;
    ...
    protected static string GetConversionGroup(Type type, IReadOnlyDictionary<Type, TypeMapping> mappings)
    {
        mappings.TryGetValue(type, out var mapping);
        if (mapping is null)
    ...
    protected static Type GetEnumeratedType(Type? type)
    {
        if (type is null)
    ...
    #endregion Support for Functions & Procedures
}