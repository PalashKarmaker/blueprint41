...
public abstract class EntityCollectionBase : IItteratable<CollectionItem>, IInternalListAccess
{
    ...
    void IItteratable<CollectionItem>.ForEach(Action<int, CollectionItem> action) => ForEach(action);
    ...
    void IInternalListAccess.EnsureLoaded() => EnsureLoaded();
    void IInternalListAccess.Add(CollectionItem item) => Add(item);
    CollectionItem? IInternalListAccess.GetItem(int index) => GetItem(index);
    void IInternalListAccess.SetItem(int index, CollectionItem item) => SetItem(index, item);
    void IInternalListAccess.RemoveAt(int index) => RemoveAt(index);
    int[] IInternalListAccess.IndexOf(OGM item) => IndexOf(item);
    ...
    private static readonly List<CollectionItem> empty = [];
    ...
    protected private Transaction RunningTransaction
    {
        get
        {
            Transaction? trans = DbTransaction ?? throw new InvalidOperationException("There is no transaction, you should create one first -> using (Transaction.Begin()) { ... Transaction.Commit(); } or you loaded this object in another transaction then you are using now.");
