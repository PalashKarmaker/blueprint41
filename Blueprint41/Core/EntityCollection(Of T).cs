    ...
    where TEntity : class, OGM
{
    ...
    internal override RelationshipAction RemoveAction(OGM item, DateTime? moment) => new RemoveRelationshipAction(RelationshipPersistenceProvider, Relationship, InItem(item), OutItem(item));
    internal override RelationshipAction AddAction(OGM item, DateTime? moment, Dictionary<string, object>? properties) => 
        new AddRelationshipAction(RelationshipPersistenceProvider, Relationship, InItem(item), OutItem(item), properties);
