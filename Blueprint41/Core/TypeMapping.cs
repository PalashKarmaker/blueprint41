...
public class TypeMapping
{
    ...
    {
        getConverter ??= typeof(Conversion<,>).MakeGenericType([ReturnType, PersistedType]).GetTypeInfo().DeclaredMethods.FirstOrDefault(item => item.Name == "Convert");
    ...
    public MethodInfo? GetSetConverter()
    {
        if (setConverter is null)
            setConverter = typeof(Conversion<,>).MakeGenericType([PersistedType, ReturnType]).GetTypeInfo().DeclaredMethods.FirstOrDefault(item => item.Name == "Convert");
