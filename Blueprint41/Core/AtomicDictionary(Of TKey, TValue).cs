    ...
    where TKey : notnull
{
    ...
    public void ReadOptionalWrite(ReadLogic readLogic, Action<IDictionary<TKey, TValue>> writeLogic)
    {
        ...
        try
        {
            readLogic.Invoke(dictionary, out bool executeWrite);
            if (executeWrite)
            {
                _lock.EnterWriteLock();
                try
    ...
    public T ReadOptionalWrite<T>(ReadLogic<T> readLogic, Func<IDictionary<TKey, TValue>, T> writeLogic)
    {
        ...
        try
        {
            T result = readLogic.Invoke(dictionary, out bool executeWrite);
            if (executeWrite)
            {
                _lock.EnterWriteLock();
                try
    ...
    public void Add(IEnumerable<KeyValuePair<TKey, TValue>> items) => Write(dict => items.ForEach(item => AtomicDictionary<TKey, TValue>.AddIfNotExists(dict, item)));
    ...
    public bool TryGetValue(TKey key, [MaybeNullWhen(false)] out TValue value)
    {
        ...
        value = Read(dict =>
        {
            retval = dict.TryGetValue(key, out var v);
            return v;
        });
    ...
    {
        ReadLogic<TValue?> rlg = (IDictionary<TKey, TValue> dictRead, out bool executeWrite) =>
        {
            executeWrite = !dictRead.TryGetValue(key, out var value);
            return value;

        };
        var wlg = (IDictionary<TKey, TValue> dictWrite) =>
        {
        ...
        return ReadOptionalWrite(rlg, wlg);
    }
    ...
    private static void AddIfNotExists(IDictionary<TKey, TValue> dict, KeyValuePair<TKey, TValue> kvp)
    {
