    ...
    where TEntity : class, OGM
{
    ...
    internal sealed override void Add(TEntity item, bool fireEvents, Dictionary<string, object>? properties) => Add(item, RunningTransaction.TransactionDate, fireEvents, properties);
    ...
    internal sealed override void AddRange(IEnumerable<TEntity> items, bool fireEvents, Dictionary<string, object>? properties) => AddRange(items, RunningTransaction.TransactionDate, fireEvents, properties);
    ...
    public void AddUnmanaged(TEntity item, DateTime? startDate, DateTime? endDate, Dictionary<string, object>? properties, bool fullyUnmanaged = false) => RunningTransaction.RelationshipPersistenceProvider.AddUnmanaged(Relationship, InItem(item), OutItem(item), startDate, endDate, properties, fullyUnmanaged);
    public sealed override bool Contains(TEntity item) => Contains(item, Transaction.Current?.TransactionDate ?? DateTime.UtcNow);
    ...
    internal sealed override void Remove(TEntity item, bool fireEvents) => Remove(item, RunningTransaction.TransactionDate, fireEvents);
    ...
    public void Remove(TEntity item, DateTime? moment) => Remove(item, moment, true);
    ...
    public void RemoveUmanaged(TEntity item, DateTime? startDate) => RunningTransaction.RelationshipPersistenceProvider.RemoveUnmanaged(Relationship, InItem(item), OutItem(item), startDate);
    internal sealed override void Clear(bool fireEvents) => Clear(RunningTransaction.TransactionDate);
    public void Clear(DateTime? moment) => Clear(moment, true);
    ...
    protected override void AddItem(TEntity item, DateTime? moment, Dictionary<string, object>? properties) => Add(item, moment, true, properties);
    ...
    internal override CollectionItem NewCollectionItem(OGM parent, OGM item, DateTime? startDate, DateTime? endDate) => new CollectionItem<TEntity>(parent, (TEntity)item, startDate, endDate);
    internal override RelationshipAction AddAction(OGM item, DateTime? moment, Dictionary<string, object>? properties) => new TimeDependentAddRelationshipAction(RelationshipPersistenceProvider, Relationship, InItem(item), OutItem(item), moment, properties);
    internal override RelationshipAction RemoveAction(OGM item, DateTime? moment) => new TimeDependentRemoveRelationshipAction(RelationshipPersistenceProvider, Relationship, InItem(item), OutItem(item), moment);
    ...
    internal RelationshipAction RemoveUnmanagedAction(OGM item, DateTime? startDate) => new TimeDependentRemoveUnmanagedRelationshipAction(RelationshipPersistenceProvider, Relationship, InItem(item), OutItem(item), startDate);
    internal RelationshipAction AddUnmanagedAction(OGM item, Dictionary<string, object>? properties, DateTime? startDate, DateTime? endDate) => new TimeDependentAddUnmanagedRelationshipAction(RelationshipPersistenceProvider, Relationship, InItem(item), OutItem(item), startDate, endDate, properties);
}
