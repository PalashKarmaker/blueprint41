...
internal sealed class CustomTaskRunner
{
    ...
    public override string ToString() => $"Runner {Id}, Thread {Thread.ManagedThreadId} ({Thread.ThreadState}), Task {RunningTask?.Task.Id.ToString() ?? "None"} ({RunningTask?.Task.Status.ToString() ?? "NA"})";
}
