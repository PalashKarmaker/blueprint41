...
public abstract class Data
{
    ...
    private static AtomicDictionary<string, Dictionary<string, PropertyDetail>> s_PropertyDetails = [];
    protected Dictionary<string, PropertyDetail> PropertyDetails
    {
        get
        {
            ...
            {
                Dictionary<string, PropertyDetail> details = [];
                ...
                {
                    var info = Array.Find(infos, item => item.Name == property.Name) ?? throw new MissingFieldException("The field {0} does not exist, please regenerate the data access classes.");
    ...
    public virtual void MapFrom(IReadOnlyDictionary<string, object?> properties)
    {
        ...
        {
            if (PropertyDetails.TryGetValue(property.Key, out PropertyDetail? info))
    ...
    public object? GetValue(string propertyName, bool throwWhenMissing = false)
    {
        ...
        if (PropertyDetails.TryGetValue(propertyName, out PropertyDetail? info))
        {
        ...
        else
        {
            if (UnidentifiedProperties is not null)
            {
                if (UnidentifiedProperties.TryGetValue(propertyName, out object? value))
                    return value;
            }
        }
    ...
    protected class PropertyDetail
    {
        ...
        private Func<Data, object?> BuildGetAccessor(PropertyInfo propertyInfo)
        {
            ...
            var convertedValue = Mapping.NeedsConversion ?
                                    Expression.Call(Mapping?.GetGetConverter()!, property) :
