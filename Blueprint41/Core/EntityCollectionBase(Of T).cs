...
{
    protected EntityCollectionBase(OGM parent, Property property, Action<TEntity>? eagerLoadLogic) : base(parent, property) => EagerLoadLogic = eagerLoadLogic;
    ...
    sealed internal override void InitialLoad(IEnumerable<CollectionItem> items) => InitialLoad(items.Cast<CollectionItem<TEntity>>());
    ...
    internal protected override void AfterFlush() => loadedData = innerData;
    ...
    internal void AddRange(IEnumerable<TEntity> items, Dictionary<string, object>? properties)
    {
        ...
        AddRange(items, typeof(TEntity) != typeof(DynamicEntity), properties);
    }
