...
public abstract class OGMImpl : OGM
{
    ...
    protected bool RelationshipExists(EntityProperty foreignProperty, OGM instance) => PersistenceProvider.NodePersistenceProvider.RelationshipExists(foreignProperty, instance);
    ...
    object? OGM.GetKey() => this.GetKey();
    void OGM.SetKey(object key) => this.GetData().SetKey(key);

    DateTime OGM.GetRowVersion() => this.GetRowVersion();

    IDictionary<string, object?> OGM.GetData() => this.GetData().MapTo();
    ...
    internal protected virtual bool LazySet<T>(Property property, T previousValue, T assignValue, DateTime? moment) => property.RaiseOnChange<T>(this, previousValue, assignValue, moment, OperationEnum.Set);
        ...
        where TEnum : struct
    {
        ...
        if (!Enum.TryParse<TEnum>(self, out TEnum result))
            return null;
