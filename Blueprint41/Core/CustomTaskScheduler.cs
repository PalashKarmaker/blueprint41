...
public class CustomTaskScheduler : TaskScheduler, IDisposable
{
    ...
    protected override void QueueTask(Task task)
    {
        if (task != null)
        {
            var customTask = CreateTask(task);
            if (customTask != null && !customTask.Scheduled)
            {
                lock (customTask)
                {
    ...
    protected CustomTask? CreateTask(Task task) => history.TryGetOrAdd(task, delegate (Task key)
                                                            {
                                                                var antecedents = key.GetAntecedentTasks()?.Select(item => CreateTask(item)).ToList();
                                                                if (antecedents is null)
                                                                {
                                                                    Task? antecedent = task.GetAntecedentTask();
                                                                    if (antecedent is not null)
                                                                        antecedents = [CreateTask(antecedent)];
                                                                }
                                                                var lst = antecedents?.Where(p => p != null).Select(p => p!).ToList();
                                                                if (CustomTask.Current is null)
                                                                    return new CustomTask(key, taskDescription.Value, lst);
                                                                else
                                                                    return new CustomTask(CustomTask.Current, task, lst);
                                                            });
...
#pragma warning restore S2696 // But here we can because its not static it's ThreadStatic
    ...
    public virtual void Wait(bool includeSubTasks = true, bool clearHistory = true, StatusUpdate? status = null)
    {
        ...
        {
            KeyValuePair<Task, CustomTask>[] items = [.. history];
