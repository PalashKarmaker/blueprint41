...
public static class AsyncHelper
{
    ...
    public static TResult RunSync<TResult>(Func<Task<TResult>> func) => _myTaskFactory
          .StartNew(func)
          .Unwrap()
          .GetAwaiter()
          .GetResult();
    ...
    [Obsolete("Please stop using this and consider using the CustomTaskScheduler instead", true)]
    public static void RunSync(Func<Task> func) => _myTaskFactory
          .StartNew(func)
          .Unwrap()
          .GetAwaiter()
          .GetResult();

    public static Task<TResult> NewTask<TResult>(Func<TResult> func) => _myTaskFactory.StartNew(func);

    public static Task NewTask(Action func) => _myTaskFactory.StartNew(func);
    ...
    public static void WaitEx(this IEnumerable<Task> tasks, bool recursive = true) => WaitEx(tasks, recursive, null);
    ...
    private static readonly AtomicDictionary<Type, Func<Task, Task?>?> getAntecedentTaskMethods = [];
    private static readonly AtomicDictionary<Type, Func<Task, IEnumerable<Task>?>?> getAntecedentTasksMethods = [];
    ...
    private static readonly AtomicDictionary<Type, Func<Task, Task?>?> getTaskResultMethods = [];
    private static readonly AtomicDictionary<Type, Func<Task, IEnumerable<Task>?>?> getTaskResultsMethods = [];
}
