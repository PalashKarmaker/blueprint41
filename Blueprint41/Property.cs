...
public abstract partial class Property : IRefactorProperty, IPropertyCondition, IPropertyEvents
{
    ...
    internal void SetParentEntity(Entity parent) => Parent = parent;
    internal void SetReturnTypeEntity(Entity returnType) => EntityReturnType = returnType;
...
#pragma warning restore S2583 // Conditionally executed code should be reachable
    ...
    private static readonly (Type fromType, Type? toType, string typeCheck, string typeConv)[] specificConvertTabel = [
    ...
    private static readonly (Type fromType, Type toType, string typeCheck, string typeConv)[] genericConvertTabel = [
        // bool
    ...
    void IRefactorProperty.MakeMandatory(object defaultValue)
    {
        ...
        if (PropertyType == PropertyType.Attribute && SystemReturnType is not null)
        {
            Nullable = false;
            foreach (var entity in Parent.GetConcreteClasses())
            {
    ...
    private class RelationshipPattern
    {
        ...
        internal void ValidateRecursive(Entity fromEntity, NodePattern node, bool strict)
        {
            ...
            {
                bool found = false;
                var lst = node.Entity?.GetRelationshipsRecursive();
                if (lst != null)
                    foreach (var relation in lst)
                    {
        ...
        internal int HasAlias(string alias) => Next?.HasAlias(alias) ?? 0;
    ...
    string IPropertyCondition.Contains(string text) => string.Format("{{0}}.{0} CONTAINS '{1}'", Name, text);
                ...
                if (getValueWithMoment is null)
                {
                    lock (this)
                    {
                        if (getValueWithMoment is null)
                        {
                            ...
                            MethodInfo? method = entity.RuntimeReturnType!.GetMethods().FirstOrDefault(item => item.Name == name) ?? throw new NotSupportedException("No get accessor exists");
            ...
            else
            {
                if (getValue is null)
                {
                    lock (this)
                    {
                        if (getValue is null)
                        {
                            MethodInfo? method = (entity.RuntimeReturnType!.GetProperties().FirstOrDefault(item => item.Name == Name)?.GetGetMethod()) ?? throw new NotSupportedException("No get accessor exists");
                ...
                if (setValueWithMoment is null)
                {
                    lock (this)
                    {
                        if (setValueWithMoment is null)
                        {
                            ...
                            MethodInfo? method = entity.RuntimeReturnType!.GetMethods().FirstOrDefault(item => item.Name == name) ?? throw new NotSupportedException("No set accessor exists");
    ...
    internal void ClearLookup(OGM instance, DateTime? moment = null)
    {
        ...
        {
            if (!clearLookupCache.TryGetValue(type, out Action<OGM, DateTime?>? clearLookup))
            {
                lock (this)
                {
    ...
    private Dictionary<Type, Action<OGM, DateTime?>> clearLookupCache = [];
    ...
    {
        if (!propertyEventArgsType.TryGetValue(senderType.Name, out var type))
        {
            lock (this)
            {
    ...
    private Dictionary<string, Type> propertyEventArgsType = [];
...
public class EntityProperty : Property, IRefactorEntityProperty
{
    ...
    void IRefactorEntityProperty.Reroute(string pattern, string newPropertyName, string newRelationshipName, string newNeo4jRelationshipType, bool strict) => Reroute(pattern, newPropertyName, newRelationshipName, newNeo4jRelationshipType ?? newRelationshipName, strict);
