...
public class DynamicEntity : DynamicObject, OGM
{
    internal DynamicEntity(Entity entity, bool shouldExecute, object? initialize = null)
    {
        ...
        {
            Property? property = DynamicEntityType.Search(init.Key) ?? throw new ArgumentException(string.Format("The property '{0}' is not contained within entity '{1}'.", init.Key, entity.Name));
            ...
            else
            {
                ...
                IEnumerable<DynamicEntity>? input = (init.Value as IEnumerable<DynamicEntity> ?? ((init.Value as IEnumerable<object>)?.Cast<DynamicEntity>())) ?? throw new InvalidCastException("should be a collection of DynamicEntity");
                if (!TryGetMember(init.Key, out object? member))
                    throw new ArgumentException(string.Format("The property '{0}' is not contained within entity '{1}'.", init.Key, entity.Name));
    ...
    internal void RefactorActionPropertyRenamed(string oldname, Property item, MergeAlgorithm mergeAlgorithm = MergeAlgorithm.NotApplicable)
    {
        if (item.PropertyType == PropertyType.Attribute)
        {
            switch (mergeAlgorithm)
            {
                ...
                case MergeAlgorithm.ThrowOnConflict:
                    {
                        if (DynamicEntityValues.TryGetValue(oldname, out object? oldvalue))
                        {
        ...
        else
        {
            switch (mergeAlgorithm)
            {
                ...
                case MergeAlgorithm.ThrowOnConflict:
                    {
                        if (DynamicEntityLinks.TryGetValue(oldname, out EntityCollectionBase<DynamicEntity>? oldvalue))
                        {
    ...
    internal void RefactorActionPropertyConverted(Property item, Type target)
    {
        ...
        if (DynamicEntityValues.TryGetValue(item.Name, out object? oldValue))
        {
            object? newValue = Conversion.Convert(item.SystemReturnType, target, oldValue);
            DynamicEntityValues[item.Name] = newValue;
        }
    }
    internal void RefactorActionPropertyRerouted(string oldname, Entity to, Property item) => throw new NotSupportedException("Rerouting of static data is not yet supported.");
    ...
    private readonly Dictionary<string, object?> DynamicEntityValues = [];
    private readonly Dictionary<string, EntityCollectionBase<DynamicEntity>> DynamicEntityLinks = [];
    public Guid Guid { get; private set; }
    public IReadOnlyDictionary<string, object?> GetDynamicEntityValues() => this.DynamicEntityValues;
    ...
    public bool TryGetMember(string propertyName, out object? result)
    {
        ...
        switch (property.PropertyType)
        {
            ...
            case PropertyType.Collection:
                {
                    DynamicEntityLinks.TryGetValue(propertyName, out EntityCollectionBase<DynamicEntity>? collection);
                    result = collection;
                    break;
                }
                ...
                case PropertyType.Lookup:
                    {
                        ...
                        if (node is not null && node.GetEntity() != property.EntityReturnType)
                            return false;

                        DynamicEntityLinks.TryGetValue(name, out EntityCollectionBase<DynamicEntity>? collection);
    ...
    void OGM.ValidateSave()
    {
        ...
        foreach (Entity inherited in DynamicEntityType.GetBaseTypesAndSelf())
        {
            foreach (Property attr in inherited.Properties)
            {
                ...
                {
                    if (!TryGetMember(attr.Name, out object? value) || value is null)
    ...
    internal object? GetKey() => ((OGM)this).GetKey();
    ...
    object? OGM.GetKey()
    {
        ...
        {
            if (!PersistenceProvider.ConvertToStoredTypeCache.TryGetValue(DynamicEntityType.Key.SystemReturnType!, out Conversion? converter))
                return key;
    ...
    void OGM.SetKey(object key)
    {
        ...
        var dynamicKeyName = DynamicEntityType.Key?.Name;
        if (dynamicKeyName is not null)
        {
            if (DynamicEntityValues.ContainsKey(dynamicKeyName))
                DynamicEntityValues[dynamicKeyName] = convertedKey;
            else
                DynamicEntityValues.Add(dynamicKeyName, convertedKey);
        }
    ...
    DateTime OGM.GetRowVersion()
    {
        ...
        if (!TryGetMember(entity.RowVersion.Name, out object? result))
    ...
    IDictionary<string, object?> OGM.GetData() => DynamicEntityValues.ToDictionary(item => item.Key, item => PersistenceProvider.ConvertToStoredType(DynamicEntityType.Search(item.Key)!.SystemReturnType!, item.Value));
    ...
    public Entity GetEntity() => DynamicEntityType;
    ...
    protected private Transaction RunningTransaction
    {
        get
        {
            Transaction? trans = DbTransaction ?? throw new InvalidOperationException("There is no transaction, you should create one first -> using (Transaction.Begin()) { ... Transaction.Commit(); }");
    ...
    protected bool RelationshipExists(EntityProperty foreignProperty, OGM instance) => RunningTransaction.NodePersistenceProvider.RelationshipExists(foreignProperty, instance);
