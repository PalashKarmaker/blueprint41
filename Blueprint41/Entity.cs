...
public partial class Entity : IRefactorEntity, IEntityEvents, ISetRuntimeType, IEntityAdvancedFeatures
{
    ...
    {
        EntityProperty? property = Search(propertyName) ?? throw new NotSupportedException("Property does not exist.");
    ...
    public Entity AddProperty(string name, string[] enumeration, bool nullable = true, IndexType indexType = IndexType.None)
    {
        ...
        EntityProperty value = new(this, PropertyType.Attribute, name, typeof(string), nullable, indexType, enumeration);
    ...
    public Entity AddProperty(string name, Type type, bool nullable, IndexType indexType = IndexType.None)
    {
        ...
        EntityProperty value = new(this, PropertyType.Attribute, name, type, nullable, indexType);
        Properties.Add(name, value);
    ...
    {
        EntityProperty value = new(this, PropertyType.Lookup, name, type, nullable, IndexType.None);
        Properties.Add(name, value);
    ...
    {
        EntityProperty value = new(this, PropertyType.Collection, name, type, true, IndexType.None);
        Properties.Add(name, value);
    ...
    void IRefactorEntity.SetDefaultValue(Action<dynamic> values)
    {
        ...
        {
            Property property = GetPropertiesOfBaseTypesAndSelf().FirstOrDefault(item => item.Name == key) ?? throw new ArgumentException(string.Format("The field '{0}' was not present on entity '{1}'. ", key, Name));
    ...
    void IRefactorEntity.CopyValue(string sourceProperty, string targetProperty)
    {
        ...
        Property? source = Search(sourceProperty) ?? throw new NotSupportedException(string.Format("The source property '{0}' was not found on entity '{1}', you cannot copy data outside of the entity with this refactor action.", sourceProperty, Name));
        Property? target = Search(targetProperty) ?? throw new NotSupportedException(string.Format("The target property '{0}' was not found on entity '{1}', you cannot copy data outside of the entity with this refactor action.", targetProperty, Name));
        if (!Parser.ShouldExecute)
            return;
    ...
    dynamic IRefactorEntity.CreateNode(object node)
    {
        ...
        DynamicEntity entity = new(this, Parser.ShouldExecute, node);
    ...
    {
        NodeEventArgs args = new(EventTypeEnum.OnNodeLoading, trans, sender, cypher, parameters, customState);
    ...
    {
        NodeEventArgs args = new(EventTypeEnum.OnBatchFinished, previousArgs);
        if (!trans.FireGraphEvents)
            return args;
    ...
    {
        NodeEventArgs args = new(EventTypeEnum.OnNodeCreate, trans, sender, cypher, parameters, customState);
        if (!trans.FireGraphEvents)
            return args;
    ...
    {
        NodeEventArgs args = new(EventTypeEnum.OnNodeCreated, previousArgs, id, labels, properties);
        if (!trans.FireGraphEvents)
            return args;
    ...
    {
        NodeEventArgs args = new(EventTypeEnum.OnNodeUpdate, trans, sender, cypher, parameters, customState);
        if (!trans.FireGraphEvents)
            return args;
    ...
    {
        NodeEventArgs args = new(EventTypeEnum.OnNodeUpdated, previousArgs);
        if (!trans.FireGraphEvents)
            return args;
    ...
    {
        NodeEventArgs args = new(EventTypeEnum.OnNodeDelete, trans, sender, cypher, parameters, customState);
        if (!trans.FireGraphEvents)
            return args;
    ...
    {
        NodeEventArgs args = new(EventTypeEnum.OnNodeDeleted, previousArgs);
        if (!trans.FireGraphEvents)
            return args;
    ...
    {
        if (baseType is not Entity baseEntity)
            return false;
    ...
    public EntityProperty? Search(string name)
    {
        if (Parent.IsUpgraded)
        {
            ...
            namedPropertiesOfBaseTypesAndSelf.TryGetValue(name.ToLower(), out EntityProperty? foundProperty);
    ...
    internal static Entity? FindCommonBaseClass(List<Entity> entities)
    {
        ...
        foreach (Entity entity in entities)
        {
            ...
            {
                HashSet<string> inheritedEntities = new(inheritChain.Select(item => item.Name));
    ...
    internal static AliasResult? FindCommonBaseClass(List<AliasResult> aliases)
    {
        ...
        foreach (AliasResultInfo info in aliases.Select(item => new AliasResultInfo(item)))
        {
            ...
            else
            {
                ...
                HashSet<string> inheritedEntities = new(inheritChain.Select(item => item.Entity.Name));
                for (int index = shared.Count - 1; index >= 0; index--)
                {
