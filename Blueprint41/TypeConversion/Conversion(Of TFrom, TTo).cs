...
public abstract class Conversion<TFrom, TTo> : Conversion
{
                ...
                if (!isInitialized)
                {
                    ...
                    if (converterMethod is null)
                    {
                        ...
                        if (fromIsNullable || toIsNullable)
                        {
                            ...
                            {
                                if (compiledConvert is null && uncompiledConvert is not null)
    ...
    {
        Type converterType = typeof(Conversion<,>).MakeGenericType([fromType, toType]);
        var canConvert = converterType.GetTypeInfo().DeclaredMethods.FirstOrDefault(item => item.Name == "CanConvert");

        var canConv = canConvert?.Invoke(null, []);
        if (canConv is null || canConv is not bool)
            throw new NotSupportedException($"You cannot convert a {typeof(TFrom).Name} to a {typeof(TTo).Name}.");

        if ((bool)canConv == true)
        {
            var convertMethodInfo = converterType.GetTypeInfo().DeclaredMethods.FirstOrDefault(item => item.Name == "Convert");
            var valuePropertyInfo = typeof(TFrom).GetTypeInfo().DeclaredProperties.FirstOrDefault(item => item.Name == "Value");
            var hasValuePropertyInfo = typeof(TFrom).GetTypeInfo().DeclaredProperties.FirstOrDefault(item => item.Name == "HasValue");
            ...
            {
                var label = Expression.Label(typeof(TTo));
    ...
    internal override void RegisterConversion() => converterMethod = Converter!;
    ...
    internal override bool IsValidConversion() => CanConvert();

    public sealed override object? Convert(object? value) => Convert((TFrom)value!);
}
...
internal class ConversionInstance<TFrom, TTo> : Conversion<TFrom, TTo>
{
    ...
    [return: MaybeNull]
    protected override TTo Converter([AllowNull] TFrom value) => throw new NotImplementedException();
